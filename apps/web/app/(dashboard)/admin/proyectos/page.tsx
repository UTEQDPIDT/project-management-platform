...
const Page = () => {
  ...
  return (
    <div className="w-full min-h-screen">
      <Header>
        <HeaderHeading>
          <HeaderTitle>{showHidden ? 'Proyectos ocultos' : 'Proyectos'}</HeaderTitle>
          <HeaderDescription>
            {showHidden
              ? 'Proyectos ocultados de los listados generales.'
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem onClick={() => setShowHidden((prev) => !prev)}>
                  {showHidden ? (
                    <>
                      <Eye />
                      Ver proyectos visibles
                    </>
                  ) : (
                    <>
                      <EyeOff />
                      Ver proyectos ocultos
                    </>
                  )}
                </DropdownMenuItem>
              </DropdownMenuContent>
      ...
      <PageContent className="flex flex-col gap-6 w-full max-w-full overflow-hidden">
        <ProjectsTable showHidden={showHidden} />
        {!showHidden && <ProductsTable />}
      </PageContent>
    </div>
  );
};
